...
uartSender = uart.uartSender = () => {
  ...
  interface.port.write(txBuf, function(err) {
    if (err) {
      ...
      showMsg("info", "Sent '" + msg + "' to connected SensorTag.");
...
parseChallenge = uart.parseChallenge = (data) => {
  ...
  if (interface.debugMode) {
    showMsg("info", "UART:" + JSON.stringify(data.toString().replace(/\x00*$/, '')));
  }
...
sendChallenge = uart.sendChallenge = () => {
  ...
  setTimeout(function() { // wait the grace period and check 'responded' after that
    if (!uart.responded) {
      ...
      portFinder.nextPort();
      interface.port.close(err => {if (err) {showMsg("error", "Port close error: "+err);}}); // activates port.on('close')
      showMsg("info", "\n");
      return;
    }
  }, 3000);
