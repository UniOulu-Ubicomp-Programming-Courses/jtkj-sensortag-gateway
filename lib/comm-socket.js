...
const socket = io.connect(interface.socket.host, interface.socket.options);
...
send = comm.send = (topic, msg) => {
  return new Promise(resolve => {
    let option = {};
    if (interface.debugMode) showMsg("debug", topic + " " + JSON.stringify(msg));
...
startComm = comm.startComm = () => {
  socket.on("connect_failed", () => {
    showMsg("error", "Connection to Backend server failed!");
  });

  socket.on("connect", () => {
    showMsg("info", "Connected to Backend server.");
  ...
  socket.on("error", err => {
    showMsg("error", "Socket reported a generic error!");
  });
}
